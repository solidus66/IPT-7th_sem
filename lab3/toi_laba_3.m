...
K=[100,200, 300, 400, 500, 600, 700, 800, 900, 1000];%количество статистических испытаний
...
for i=1:M,
    for j=i+1:M,
           ...
           trij_2=trace((C_(:,:,j)*C(:,:,i)-eye(n))^2); trji_2=trace((eye(n)-C_(:,:,i)*C(:,:,j))^2);
...
Pc_exp = zeros(1, numel(K));%экспериментальная ошибка второго рода для второго класса
Pc_theor = PIJ(2, 2); %теоретическая ошибка второго рода для второго 
for idx = 1:numel(K)%экспериментальная матрица вероятностей ошибок
    ...
    Pc_exp(idx) = Pc_(2, 2) / K(idx);%экспериментальная ошибка второго рода для второго класса
end;
...
if n==2,
      ...
      disp('Оценки суммарных ошибок'); disp([Es1,Es2,Es3]);%отображение оценок суммарных ошибок
      xmin1=-3*sqrt(max(D(1,:)))+min(m(1,:)); xmax1=3*sqrt(max(D(1,:)))+max(m(1,:));
      xmin2=-3*sqrt(max(D(2,:)))+min(m(2,:)); xmax2=3*sqrt(max(D(2,:)))+max(m(2,:));
      x1=xmin1:0.1:xmax1; x2=xmin2:0.1:xmax2;
      ...
      for i=1:M,
          ...
          for j=i+1:M,%изображение разделяющих границ
              ...
              fd=-0.5*(C_(:,:,i)-C_(:,:,j))*x12';   f4=f4+sum(x12'.*fd);
...
% Построение графика экспериментальной ошибки первого рода для второго класса по числу испытаний
figure(2);
plot(K, Pc_exp, 'b'); hold on;
plot(K, Pc_theor*ones(size(K)), 'r'); 
xlabel('Число испытаний (объем выборки)'); ylabel('Ошибка второго рода');
title('Зависимость экспериментальной и теоретической ошибки второго рода для второго класса от числа испытаний');
legend('Экспериментальная ошибка', 'Теоретическая ошибка'); grid on;

% Вывод теоретического и экспериментального значения ошибки первого рода для второго класса
disp("Теоретическое значение: " + PIJ(2, 2)); 
disp("Экспериментальное значение: " + Pc_exp(end));
